...
def run_edit_with_pipeline(spec: dict, emitter: EventEmitter, pipeline: object) -> int:
    ...
    # Note: --condition-image is not supported for edit mode. These pipelines
    # use a single `image` parameter for both VAE and vision encoder input.
    # Split routing for edit would require manual VAE encoding + prompt_embeds
    # injection (like mask_blend_loop.py does for generate). Not implemented yet.
    if condition_image_path:
        emitter.warning("CONDITION_IMAGE_UNSUPPORTED",
                        "--condition-image is not yet supported for edit mode; ignoring")
