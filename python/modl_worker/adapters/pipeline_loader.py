...
def _load_safetensors_lenient(filepath: str) -> dict:
    ...
    Strategy: truncate a temporary copy of the file to the covered range,
    then load with the standard (memory-efficient) safetensors loader.
    If the file is too large to copy, fall back to per-tensor mmap loading.
    """
    import struct
    import torch
    ...
    # Truncate in-place — removes trailing junk bytes so the strict
    # safetensors loader can handle the file from now on.
